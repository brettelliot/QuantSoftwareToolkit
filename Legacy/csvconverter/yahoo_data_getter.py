...
#    print str ("ADJ_CL: " + str(adj_close))
    ...
    _list = [(float(x)) for x in _list] #convert strings to floats
    
    try:
        ratio= adj_close/ _list[-1] #Adjusted close / close
        _list= [(x * ratio) for x in _list] #Updating the values
...
def get_data_for_exchange (exchange, data_path):
    ...
    try:
        nasdaq_params= urllib.parse.urlencode ({'exchange':str(exchange), 'render':'download'})
        nasdaq_get= urllib.request.urlopen ('http://www.nasdaq.com/screening/companies-by-name.aspx?%s' % nasdaq_params)
        ...
        symbol_list = [(x.partition(str("\""))[2]) for x in symbol_list] #Keep the stuff only after the first "
        symbol_list = [(x.partition(str("\""))[0]) for x in symbol_list] #Keep the stuff before the second "
    ...
    for symbol in symbol_list:
        ...
        try:
            params= urllib.parse.urlencode ({'a':0o3, 'b':12, 'c':2000, 'd':_now.month, 'e':_now.day, 'f':_now.year, 's': str(symbol)})
            url_get= urllib.request.urlopen("http://ichart.finance.yahoo.com/table.csv?%s" % params)
...
#                print str(symbol_data[-1])
            ...
            symbol_data= list(map (adjust, symbol_data))
            
            #Following changes so that the data looks like Norgate data and the change to cav_to_pkl.csv is minimized
            symbol_data = [(x.replace("-", "")) for x in symbol_data] 
            symbol_data = [(str(symbol) + "," + str(x)) for x in symbol_data] #This means that the header is wrong but since it is ignored later anyways- this will work
